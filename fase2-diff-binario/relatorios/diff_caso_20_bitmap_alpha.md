# Diff ZCF: `caso_00_base.cdr` vs `caso_20_bitmap_alpha.cdr`
...
### `content/data/Bitmaps.dat` — NOVO na variante (3591220 bytes)
```
           0  01 00 00 00 01 00 00 00 55 49 00 00 2c cc 36 00  ........UI..,.6.
          16  00 00 00 00 20 00 00 00 02 19 29 00 00 00 00 00  .... .....).....
...
### `META-INF/textinfo.xml` — ALTERADO (673 -> 726 bytes)

Tamanhos diferentes (delta +53). Prefixo comum: 353 bytes; sufixo comum: 320 bytes.
Miolo divergente: base [353..353) = 0 bytes, variante [353..406) = 53 bytes.
...
variante (inicio do miolo):
         ...
         369  65 5f 61 6c 70 68 61 2e 70 6e 67 3c 2f 72 64 66  e_alpha.png</rdf
         385  3a 6c 69 3e 0a 20 20 20 20 20 20 20 20 20 20 20  :li>.           
         401  20 20 20 20 20 3c 2f 72 64 66 3a 42 61 67 3e 0a       </rdf:Bag>.
         417  20 20 20 20 20                                        
```
...
variante (inicio do miolo):
          22  0a 3c 70 61 6c 65 74 74 65 20 67 75 69 64 3d 22  .<palette guid="
          38  36 37 31 36 31 65 66 64 2d 61 66 39 30 2d 34 36  67161efd-af90-46
          54  39 62 2d 62 38 65 66 2d 64 34 65 30 30 63 37 39  9b-b8ef-d4e00c79
          70  35 66 35 66 22 20 6e 61 6d 65 3d 22 50 61 6c 65  5f5f" name="Pale
          86  74 61 20 64 65 20 64 6f 63 75 6d 65 6e 74 6f 73  ta de documentos
         102  22 3e 3c 63 6f 6c 6f 72 73 3e 3c 70 61 67 65 3e  "><colors><page>
  ... (regiao truncada; tamanho real 96 bytes)
```
...
### `content/data/page1.dat` — ALTERADO (1545 -> 2696 bytes)

Tamanhos diferentes (delta +1151). Prefixo comum: 16 bytes; sufixo comum: 43 bytes.
Miolo divergente: base [16..1502) = 1486 bytes, variante [16..2653) = 2637 bytes.
...
variante (inicio do miolo):
          ...
          80  00 00 00 00 34 cc 36 00 00 00 00 00 00 00 00 00  ....4.6.........
  ... (regiao truncada; tamanho real 96 bytes)
```

### `previews/page1.png` — ALTERADO (829 -> 3001 bytes)

Tamanhos diferentes (delta +2172). Prefixo comum: 85 bytes; sufixo comum: 12 bytes.
Miolo divergente: base [85..817) = 732 bytes, variante [85..2989) = 2904 bytes.
...
variante (inicio do miolo):
          69  73 00 00 0e c3 00 00 0e c3 01 c7 6f a8 64 00 00  s..........o.d..
          85  0b 4e 49 44 41 54 78 5e ed dc 79 4c 94 77 1a c0  .NIDATx^..yL.w..
         101  f1 c6 b6 49 b3 d9 ed 6e 37 dd ec f6 b2 ae 6b 07  ...I...n7.....k.
         117  ab d4 a2 d4 7a a2 58 ea c1 a1 8c 20 88 22 c8 04  ....z.X.... ."..
         133  86 63 18 11 90 5b 60 38 05 ca 25 16 0f ac 4a ad  .c...[`8..%...J.
         149  4a b5 b4 94 8a 56 2d a5 ba 88 a8 58 8e 8a e2 b6  J....V-....X....
  ... (regiao truncada; tamanho real 96 bytes)
```

### `previews/thumbnail.png` — ALTERADO (935 -> 8890 bytes)

Tamanhos diferentes (delta +7955). Prefixo comum: 19 bytes; sufixo comum: 12 bytes.
Miolo divergente: base [19..923) = 904 bytes, variante [19..8878) = 8859 bytes.
...
variante (inicio do miolo):
          ...
          51  41 4d 41 00 00 b1 8f 0b fc 61 05 00 00 00 09 70  AMA......a.....p
          67  48 59 73 00 00 0e c3 00 00 0e c3 01 c7 6f a8 64  HYs..........o.d
          83  00 00 22 4f 49 44 41 54 78 5e ed dd 87 57 5b 67  .."OIDATx^...W[g
  ... (regiao truncada; tamanho real 96 bytes)
```
...
**Regiao 1: offset 17343, 35 byte(s)**
```
base:
...
variante:
       17327  20 20 20 20 20 20 20 22 67 75 69 64 22 3a 20 22         "guid": "
       17343  35 65 38 61 63 66 61 65 2d 31 63 66 38 2d 34 36  5e8acfae-1cf8-46
       17359  66 31 2d 39 61 31 38 2d 30 61 38 30 65 36 39 39  f1-9a18-0a80e699
       17375  35 33 37 63 22 2c 0a 20 20 20 20 20 20 20 20 22  537c",.        "
       17391  6e 61 6d                                         nam
```
