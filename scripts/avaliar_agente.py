"""Mede o agente (fase 3) com um ou mais modelos do OpenRouter nos pacotes revisados.
...
Uso:
    ...
    python scripts/avaliar_agente.py "$env:USERPROFILE\Documents\LeitorPedidosCDR\Relatorios" `
        --modelos anthropic/claude-sonnet-5 google/gemini-3.5-flash --rastros saida-agente
...
def main() -> None:
    ...
    def caminho_agente(modelo: str):
        def executar(cdr: Path, _pacote) -> dict:
            try:
                resultado = interpretar_com_agente(cdr, chave, modelo, fatos=fatos(cdr))
            ...
            print(
                ...
                f"{rastro.get('duracao_s')} s, US$ {rastro.get('custo_usd', 0):.4f}", flush=True,
            )
            if args.rastros:
